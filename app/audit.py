...
from .config import consultant, settings
...
async def run_audit(url: str, *, max_pages: int | None = None,
    ...
    # label each finding for its screenshot, and attach the real source code for
    # anything that cannot be seen on the page
    kw_by_url = {p.url: p.primary for p in kw_profile.pages if p.primary}
    page_by_url = {(p.final_url or p.url): p for p in pages}
    for f in findings:
        f.annotation = client_report.annotation_for(
            f.code, kw_by_url.get(f.url or "", "") or (kw_profile.primary_keywords or [""])[0])
        f.code_snippet, f.snippet_mark = client_report.code_snippet_for(
            f, page_by_url.get(f.url or ""))
    ...
    issue_groups = client_report.build_groups(
        findings, kw_by_url, kw_profile.brand, kw_profile.location)
    ...
    payload: dict[str, Any] = {
        ...
        "client_view": {
            "business_summary": client_report.business_summary(
                kw_profile.brand, kw_profile.services, kw_profile.location,
                home.meta_description),
            "business_bullets": client_report.business_bullets(
                kw_profile.brand, kw_profile.services, kw_profile.location,
                home.meta_description, len([p for p in pages if p.html]),
                kw_profile.primary_keywords),
            "issue_groups": [g.to_dict() for g in issue_groups],
            "consultant": {
                "name": consultant.name, "title": consultant.title,
                "years": consultant.years, "credentials": consultant.credentials,
                "phone": consultant.phone, "email": consultant.email,
                "location": consultant.location, "website": consultant.website,
                "closing": consultant.closing, "photo": consultant.photo,
            },
        },
        "generated_assets": {
